...
long allOddBits(long x) {
    // if ((x & 0xAAAAAAAAAAAAAAAA) == 0xAAAAAAAAAAAAAAAA) return 1L;
    // return 0L;
    int s = 1;
    while(s < 64){
        if (x >> s & 1L == 1L) s = s+2;
...
int floatIsLess(unsigned uf, unsigned ug) {
    ...
    unsigned signF = uf & signMask;
    unsigned signG = ug & signMask;

    unsigned eF = uf & eMask;
    unsigned eG = ug & eMask;

    unsigned fF = uf & fMask;
    unsigned fG = ug & fMask;
    ...
    if (eF != eG) {
        return (signF == 0) ^ (eF < eG);
    }

    return (signF == 0) ^ (fF < fG);
}
/*
...
//    unsigned sMask = 0x80000000;
    ...
    unsigned sF = ((uf & sMask) >> 31) & 1;
    unsigned sG = ((ug & sMask) >> 31) & 1;

    unsigned expF = (uf & eMask) >> 23;
    unsigned eF = expF - bias;
    unsigned expG = (ug & eMask) >> 23;
    unsigned eG = expG - bias;

    unsigned fF = uf & fMask;
    unsigned fG = ug & fMask;

    unsigned vF = ((-1)**sF) * fF * 2**eF;
    //sF, eF, fF
    ...
    if (eF > 0x00800000) eF -= 1;
    else if (eF == 0x00800000) fF = (uf & specialmask) >> 1;
    else fF >> 1;
