...
static const size_t class_size = 20;

/**
 * @brief the default size when extending the heap
...
/**
 * @brief Finds the next consecutive block's header on the heap.
 * 
 * @param[in] block A block in the heap
 * @return The next consecutive block's header on the heap
 * @pre The block is not the epilogue
 */
static word_t *find_next_header(block_t *block) {
    return &(find_next(block)->header);
}
...
static block_t *extend_heap(size_t size) {
    ...
    if ((bp = mem_sbrk((intptr_t)size)) == (void *) - 1) {
        dbg_printf("extend_heap returns NULL\n");
        return NULL;
    }
...
bool mm_checkheap(int line) {
    ...
    // a free block don’t have another free block in the previous/next blocks
    // headers and footers have the same allocation value
    // the size of the header should be equal to the size of the foote
...
bool mm_init(void) {
    ...
    word_t *start = (word_t *)(mem_sbrk((2 + class_size) * wsize));
    ...
    start[class_size] = pack(0, true); // Heap prologue (block footer)
    start[class_size + 1] = pack(0, true); // Heap epilogue (block header)

    // Heap starts with first "block header", currently the epilogue
    heap_start = (block_t *)&(start[class_size + 1]);
